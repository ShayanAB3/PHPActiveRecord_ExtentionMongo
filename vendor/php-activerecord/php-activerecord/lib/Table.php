...
class Table
{
	...
	public function options_to_sql($options)
	{
		...
		$sql = $this->conn->getBuilder($table);
	...
	public function find_by_sql($sql, $values=null, $readonly=false, $includes=null)
	{
		...
		foreach ($sth as $row)
		{
			...
			{
				$key = $this->cache_key_for_model(array_intersect_key($sth, array_flip($this->pk)));
	...
	public function insert(&$data, $pk=null, $sequence_name=null)
	{
		...
		$sql = $this->conn->getBuilder($this->get_fully_qualified_table_name());
		$sql->insert($data,$pk,$sequence_name);
		$values = $this->conn->getValues($data);
