...
class Model
{
	...
	private function insert($validate=true)
	{
		...
		// if (count($pk) == 1)
		{
			...
			if ($column->auto_increment || $use_sequence)
				...
				$table->sequence = $table->conn->protocol === "mongodb" ? $table->table : $table->sequence;
	...
	public static function delete_all($options=array())
	{
		...
		$sql = $conn->getBuilder($table->get_fully_qualified_table_name());

		$conditions = is_array($options) ? $options['conditions'] : $options;

		if (is_array($conditions) && !static::is_hash($conditions))
	...
	public static function update_all($options=array())
	{
		$table = static::table();
		$conn = static::connection();
		$sql = $conn->getBuilder($conn, $table->get_fully_qualified_table_name());
	...
	private function set_attributes_via_mass_assignment(array &$attributes, $guard_attributes)
	{
		...
		foreach ($attributes as $name => $value)
		{
			...
			if($connection instanceof MongodbAdapter){
	...
	public static function __callStatic($method, $args)
	{
		...
		$create = false;
		$connection = static::connection();
		$table = static::table()->get_fully_qualified_table_name();
		...
		if (substr($method,0,7) === 'find_by')
		{
			...
			$options['conditions'] = $connection->getBuilder($table)::create_conditions_from_underscored_string($connection,$attributes,$args,static::$alias_attribute);

			if (!($ret = static::find('first',$options)) && $create)
				return static::create($connection->getBuilder($table)::create_hash_from_underscored_string($attributes,$args,static::$alias_attribute));
			
			return $ret;
		}
		elseif (substr($method,0,11) === 'find_all_by')
		{
			$options['conditions'] = $connection->getBuilder($table)::create_conditions_from_underscored_string(static::connection(),substr($method,12),$args,static::$alias_attribute);
			return static::find('all',$options);
		}
		elseif (substr($method,0,8) === 'count_by')
		{
			$options['conditions'] = $connection->getBuilder($table)::create_conditions_from_underscored_string(static::connection(),substr($method,9),$args,static::$alias_attribute);
	...
	public static function find_by_pk($values, $options)
	{
		...
		if ($results != ($expected = @count((array)$values))) //count($values)
		{
			$class = get_called_class();
			if (is_array($values))
				$values = join(',',$values);
